...
exports.unblockProvider = unblockProvider;
exports.isUnblockConfigured = isUnblockConfigured;
exports.unblockRequest = unblockRequest;
exports.fetchThrough = fetchThrough;
...
function unblockProvider(options = {}) {
    const named = options.provider ?? process.env.UNBLOCK_PROVIDER;
    if (named === 'scraperapi' || named === 'brightdata' || named === 'direct')
        return named;
    if (named)
        return null;
    ...
    if (options.brightDataToken ?? process.env.BRIGHT_DATA_KEY)
        return 'brightdata';
    if (options.scraperApiKey ?? process.env.SCRAPER_API_KEY)
        return 'scraperapi';
    return null;
}
function isUnblockConfigured(options = {}) {
    const provider = unblockProvider(options);
    if (provider === 'direct')
        return true;
    if (provider === 'brightdata')
        return Boolean(options.brightDataToken ?? process.env.BRIGHT_DATA_KEY);
    if (provider === 'scraperapi')
        return Boolean(options.scraperApiKey ?? process.env.SCRAPER_API_KEY);
    return false;
}
/**
 ...
 * tested for free.
 */
function unblockRequest(provider, url, options = {}) {
    ...
    if (provider === 'scraperapi') {
        ...
        if (Object.keys(headers).length > 0)
            query.set('keep_headers', 'true');
        if (options.countryCode)
            query.set('country_code', options.countryCode);
...
async function fetchThrough(url, options = {}) {
    const provider = unblockProvider(options);
    if (!provider) {
        throw new instagram_1.ScraperError('No unblocking provider configured. Set BRIGHT_DATA_KEY or SCRAPER_API_KEY.', 'unconfigured');
    }
    if (provider !== 'direct' && !isUnblockConfigured({ ...options, provider })) {
        throw new instagram_1.ScraperError(`${provider} was chosen but its credential is missing`, 'unconfigured');
    }
    ...
    const response = await fetch(request.url, {
        ...
        throw new instagram_1.ScraperError(`${provider} did not answer for ${url}: ${err instanceof Error ? err.message : String(err)}`, 'timeout');
    });
    if (!response.ok) {
        const body = await response.text().catch(() => '');
        throw new instagram_1.ScraperError(`${provider} returned ${response.status} for ${url}: ${body.slice(0, 200)}`, 
