 ...
 */
function client(options: AnthropicOptions = {}): Anthropic {
...
function models(options: AnthropicOptions) {
  return {
    primary: options.primaryModel ?? process.env.AI_PRIMARY_MODEL ?? DEFAULT_PRIMARY_MODEL,
    escalation:
      options.escalationModel ?? process.env.AI_ESCALATION_MODEL ?? DEFAULT_ESCALATION_MODEL,
  };
}
...
export const SUPPORTED_MEDIA = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'] as const;
...
export function isSupportedMedia(mediaType: string): mediaType is SupportedMedia {
  return (SUPPORTED_MEDIA as readonly string[]).includes(mediaType);
}
...
function buildUserPrompt(caption: string, postDate: string): string {
...
async function fetchImageAsBase64(imageUrl: string): Promise<InlineImage | null> {
  try {
    ...
    console.warn(`[mkevents] Image fetch failed for ${imageUrl}:`, (err as Error).message);
    return null;
  }
}
...
async function extract(
  ...
  const response = await anthropic.messages.parse({
    ...
    output_config: { format: zodOutputFormat(EventSchema) },
  });

  return response.parsed_output ?? null;
}

export async function parseEventFromPost(
  imageUrl: string,
  caption: string,
  postDate: string,
...
export async function parseEventFromImage(
  ...
  if (!isUncertain || escalation === primary) return parsed;

  console.log(
    `[mkevents] Escalating to ${escalation} (${primary} confidence=${parsed.confidence})`,
  );

  try {
    return (await extract(anthropic, escalation, image, caption, postDate)) ?? parsed;
  } catch (err: unknown) {
    console.warn('[mkevents] Escalation failed, keeping primary result:', (err as Error).message);
    return parsed;
  }
}
